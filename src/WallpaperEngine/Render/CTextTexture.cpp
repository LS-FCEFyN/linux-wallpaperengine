...
void CTextTexture::rebuild (FT_Face face, const std::string& text) {
        ...
        for (unsigned int row = 0; row < bmp.rows; ++row) {
            for (unsigned int col = 0; col < bmp.width; ++col) {
                ...
                if (dstX < 0 || dstX >= width || dstY < 0 || dstY >= height)
                    continue;
                pixels[static_cast<size_t> (dstY) * width + dstX] =
    ...
    // Resolution vec4: (textureW, textureH, realW, realH).
    // Text has no separate "real" vs "texture" concept, so they are equal.
    m_resolution = { static_cast<float> (width),  static_cast<float> (height),
...
// ─────────────────────────────────────────────────────────────────────────────
GLuint CTextTexture::getTextureID (uint32_t /*imageIndex*/) const {
    return m_glTexture;
}

uint32_t CTextTexture::getTextureWidth (uint32_t /*imageIndex*/) const {
    return m_width;
}

uint32_t CTextTexture::getTextureHeight (uint32_t /*imageIndex*/) const {
    return m_height;
}
...
const glm::vec4* CTextTexture::getResolution () const {
    return &m_resolution;
}
